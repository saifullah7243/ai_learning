import sys
import os
import asyncio
from dotenv import load_dotenv

# Step 1: Add parent folder (project root) to sys.path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))

# Step 2: Now import works fine
from utils.config_loader import load_config

# Load .env variables and config
load_dotenv()
config = load_config()
openai_key = config.openai_key

from langchain_openai import ChatOpenAI
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder
from langchain_core.runnables.history import RunnableWithMessageHistory
from langchain_community.chat_message_histories import ChatMessageHistory
from langchain_core.chat_history import BaseChatMessageHistory

# 1️ Define the core chain
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a helpful assistant. Answer all questions to the best of your ability."),
    MessagesPlaceholder(variable_name="history"),
    ("human", "{input}")
])

model = ChatOpenAI(model="gpt-4.1-nano", temperature=0.3)

chain = prompt | model

# 2️ Session History Management
store = {}

def get_session_history(session_id: str) -> BaseChatMessageHistory:
    if session_id not in store:
        store[session_id] = ChatMessageHistory()
    return store[session_id]

# 3️ Wrap the original chain with memory
with_message_history = RunnableWithMessageHistory(
    chain,
    get_session_history=get_session_history,  
    input_messages_key="input",
    history_messages_key="history"  
)

# 4️ Interact with chain
config_user_1 = {"configurable": {"session_id": "user_1"}}

async def run_conversation():
    # First message from user_1
    response = await with_message_history.ainvoke(
        {"input": "Hi! I'm Saif from Lahore."},
        config=config_user_1
    )
    print(f"AI: {response.content}")

    # Second message from user_1
    response = await with_message_history.ainvoke(
        {"input": "What's my name and where am I from?"},
        config=config_user_1
    )
    print(f"AI: {response.content}")

    # Now, let's interact as a different user, "user_2"
    config_user_2 = {"configurable": {"session_id": "user_2"}}

    response = await with_message_history.ainvoke(
        {"input": "Hi! I'm Sarah."},
        config=config_user_2
    )
    print(f"AI: {response.content}")

    # Ask user_2's name
    response = await with_message_history.ainvoke(
        {"input": "What is my name?"},
        config=config_user_2
    )
    print(f"AI: {response.content}")  # Should say Sarah

    # Show history for both users
    print("\nUser 1 History:")
    print(store["user_1"].messages)

    print("\nUser 2 History:")
    print(store["user_2"].messages)

if __name__ == "__main__" and os.getenv("OPENAI_API_KEY"):
    asyncio.run(run_conversation())
